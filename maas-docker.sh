...
# uncomment to see more debug info
...
case "$1" in
	...
	update)
		...
		# this is a dirty fix since packer docker-tag is broken
		signature=`$docker images -q | head -n 1`
		$docker tag -f $signature $image:latest
		;;
	updateall)
